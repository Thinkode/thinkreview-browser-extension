...
/**
 * Wraps contiguous sequences of <li> elements (generated by markdown parsing)
 * into their appropriate <ul> or <ol> parent tags based on their data-list-type attribute.
 */
function wrapListItems(html) {
  let wrapped = html;
  
  // Wrap sequences of UL list items into <ul>
  wrapped = wrapped.replace(/(?:<br>)*(<li data-list-type="ul">[\s\S]*?<\/li>)(?:(?:<br>)+<li data-list-type="ul">[\s\S]*?<\/li>)+(?:<br>)*/g,
    (m) => `<ul>${m.replace(/<br>/g, '')}</ul>`);
  wrapped = wrapped.replace(/(^|<br>)(<li data-list-type="ul">[\s\S]*?<\/li>)(?=$|<br>)/g, (m, pre, item) => `${pre}<ul>${item}</ul>`);

  // Wrap sequences of OL list items into <ol>
  wrapped = wrapped.replace(/(?:<br>)*(<li data-list-type="ol">[\s\S]*?<\/li>)(?:(?:<br>)+<li data-list-type="ol">[\s\S]*?<\/li>)+(?:<br>)*/g,
    (m) => `<ol>${m.replace(/<br>/g, '')}</ol>`);
  wrapped = wrapped.replace(/(^|<br>)(<li data-list-type="ol">[\s\S]*?<\/li>)(?=$|<br>)/g, (m, pre, item) => `${pre}<ol>${item}</ol>`);
