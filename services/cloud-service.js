...
export class CloudService {
  ...
  static async reviewPatchCode(patchContent, language = 'English', mrId = null, mrUrl = null, forceRegenerate = false, platform = null) {
    ...
    try {
      ...
      if (language && language !== 'English') { requestBody.language = language; }
      ...
      try {
        const platformTokens = await new Promise((resolve) => { chrome.storage.local.get(['gitlabToken', 'githubToken', 'bitbucketToken', 'azureDevOpsToken'], resolve); });
        let userAccessToken = null;
        if (platform === 'gitlab') userAccessToken = platformTokens.gitlabToken;
        else if (platform === 'github') userAccessToken = platformTokens.githubToken;
        else if (platform === 'bitbucket') userAccessToken = platformTokens.bitbucketToken;
        else if (platform === 'azure-devops') userAccessToken = platformTokens.azureDevOpsToken;
        requestBody.gitContext = { platform: platform || 'unknown', repositoryUrl: mrUrl || null, userAccessToken: userAccessToken || null };
      } catch (e) { dbgWarn('Failed to construct gitContext for review:', e); }
  ...
  static async getConversationalResponse(patchContent, conversationHistory, mrId = null, mrUrl = null, language = 'English') {
    ...
    try {
      ...
      if (mrUrl) { requestBody.mrUrl = mrUrl; }
      if (email) { requestBody.email = email; }
      if (mrId) { requestBody.mrId = mrId; }
      if (language && language !== 'English') { requestBody.language = language; }
      dbgLog('Sending conversational review request with tracking:', { hasEmail: !!email, hasMrId: !!mrId, hasMrUrl: !!mrUrl, conversationHistoryLength: conversationHistory.length, language: language });
      try {
        const platformTokens = await new Promise((resolve) => { chrome.storage.local.get(['gitlabToken', 'githubToken', 'bitbucketToken', 'azureDevOpsToken', 'platform'], resolve); });
        let platform = platformTokens.platform;
        if (!platform && mrUrl) {
          if (mrUrl.includes('gitlab')) platform = 'gitlab';
          else if (mrUrl.includes('github')) platform = 'github';
          else if (mrUrl.includes('bitbucket')) platform = 'bitbucket';
          else if (mrUrl.includes('dev.azure')) platform = 'azure-devops';
        }
        let userAccessToken = null;
        if (platform === 'gitlab') userAccessToken = platformTokens.gitlabToken;
        else if (platform === 'github') userAccessToken = platformTokens.githubToken;
        ...
        requestBody.gitContext = { platform: platform || 'unknown', repositoryUrl: mrUrl || null, userAccessToken: userAccessToken || null };
      } catch (e) { dbgWarn('Failed to construct gitContext for conversational review:', e); }
